...
# Global State for custom encoding & Fonts
USER_ENCODE_STATE = {}
...
# MODIFIED SIGNATURE TO ACCEPT INTERACTIVE INPUTS
async def encode(filepath, message, msg, audio_map=None, custom_sub_path=None, sub_mode=None, custom_quality=None):
    ...
    # COPY SUBTITLES & CUSTOM SUBTITLE LOGIC
    ...
    try:
        ...
        pass # Handle if DB func is missing
...
# =========================================================
# 🔴 NEW AUTHENTICATION & INTERACTIVE HANDLERS
# =========================================================
...
async def add_custom_font(client, message):
    ...
    try:
        ...
        await status.edit_text(f"✅ Your font **{font_name}** has been saved to the Database!")
    except Exception as e:
        await status.edit_text("❌ Method `add_font` not found in database.py yet.")
...
async def remove_font(client, message):
    ...
    try:
        ...
        await message.reply_text("🗑 Custom font has been deactivated. Default font will be used.")
    except:
        await message.reply_text("❌ Method `remove_active_font` not found in database.py yet.")
...
async def interactive_encode_quality(client, callback_query):
    ...
    if user_id not in USER_ENCODE_STATE: 
        return await callback_query.answer("⚠️ Session Expired. Please send the command again.", show_alert=True)
